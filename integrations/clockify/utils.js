...
export async function setupFilesInHomeAndPromptForInfo() {
  const bootstrapFiles = async () => {
    fs.mkdir(CONFIG_DIR, { recursive: true }, (error) => {
      if (error)
        throw new Error(
          `Error when attempting to create directory in ${CONFIG_DIR}: ${error}`,
        );
    });
  ...
  const question = (string) =>
    new Promise((resolve) => rl.question(string, resolve));

  setConfig(currConfig);
  console.log();
  console.log(
    `Config \x1b[32msuccessfully\x1b[m updated at \x1b[33m${CONFIG_FILE}\x1b[m`,
  );
  console.log();
  console.log(
    `If something crashes, make sure that the config values makes sense:`,
  );
...
export async function* timeEntryGenerator(headers, from, to) {
  try {
    ...
    console.log(
      `\x1b[31mAn error occured when attempting to get data from Harvest\x1b[0m`,
    );
